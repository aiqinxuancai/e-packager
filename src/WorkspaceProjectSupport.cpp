...
std::string BuildAgentsMarkdown(const SourceFileInfo& info)
{
	...
	stream
		...
		<< "## 项目说明\r\n"
		<< "\r\n"
		<< "当前目录是由 `" << info.fileName << "` 解包得到的易语言目录工程。\r\n"
		<< "外部编辑器应直接修改本目录内容，再通过 `tool\\\\e-packager.exe` 回包生成 `.e` 文件。\r\n"
		<< "\r\n"
		<< "## 目录结构\r\n"
		<< "\r\n"
		...
		<< "- `image/`：图片资源及 `list.json`。\r\n"
		<< "- `audio/`：音频资源及 `list.json`。\r\n"
		<< "- `tool/`：当前目录自带的 `e-packager.exe`。\r\n"
		<< "- `info.json`：记录本目录来源 `.e` 的文件名、路径、修改时间、尺寸、MD5。\r\n"
		<< "\r\n"
		<< "## 回包方法\r\n"
		<< "\r\n"
		<< "在项目根目录执行以下任一方式：\r\n"
		...
		<< "  默认输出到 `pack/` 目录，文件名优先使用 `info.json` 中记录的原始文件名。\r\n"
		<< "- 显式方式：`tool\\\\e-packager.exe pack . .\\\\pack\\\\" << info.fileName << "`\r\n"
		<< "\r\n"
		<< "回包后的 `.e` 可直接在易语言 IDE 中打开并编译。\r\n"
		<< "\r\n"
		<< "## 错误定位\r\n"
		<< "\r\n"
