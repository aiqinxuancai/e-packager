...
若 `ProjectBundle` 含有窗体文件或窗口绑定，编译器会读取窗口 XML、建立控件父子树、解析公开扩展属性和事件绑定，并生成 Win32 宿主。只有明确映射到 Win32 控件类的控件会进入窗口模型；无法映射的第三方控件及其子树会直接跳过，不生成容器、占位控件或伪造的私有绘制和事件实现。当前明确排除：数据报、客户、服务器、端口、表格、数据源、通用提供者、数据库提供者、图形按钮、外部数据库、外部数据提供者。
